...
# Fetch CAMD
if not os.path.isfile(os.path.join(DATA_DIR, "camd.pickle")):
    ...
    dataframe = pd.DataFrame([doc for doc in tqdm(data)])
    dataframe['structure'] = dataframe.structure.apply(Structure.from_dict)
    dataframe['nelements'] = [len(s.composition) for s in dataframe.structure]
    dataframe['nsites'] = [len(s) for s in dataframe.structure]
    dataframe.to_pickle(os.path.join("camd.pickle"))
else:
    dataframe = pd.read_pickle(os.path.join("camd.pickle"))
    # dataframe['nelements'] = [len(s.composition) for s in dataframe.structure]
    # dataframe['nsites'] = [len(s) for s in dataframe.structure]
    # dataframe.to_pickle(os.path.join("camd.pickle"))

# Filters
dataframe = dataframe[dataframe['nsites'] <= 40]
dataframe = dataframe[dataframe['nelements'] <= 4]
dataframe = dataframe[dataframe['nelements'] >= 3]
dataframe = dataframe[dataframe['stability'] <= 0.2]

# Obtain FTCP representation
if not os.path.isfile(os.path.join(DATA_DIR, "camd_ftcp.npy")):
    FTCP_representation, Nsites = FTCP_represent(dataframe, max_elms, max_sites, return_Nsites=True, camd=True)
    np.save(os.path.join(DATA_DIR, "camd_ftcp.npy"), FTCP_representation)
    np.save(os.path.join(DATA_DIR, "camd_nsites.npy"), Nsites)
else:
    FTCP_representation = np.load(os.path.join(DATA_DIR, "camd_ftcp.npy"))
    Nsites = np.load(os.path.join(DATA_DIR, "camd_nsites.npy"))
...
VAE.save(os.path.join(DATA_DIR, "camd_model"))
...
s1 = ax[1].scatter(train_latent[:,0], train_latent[:,1], s=7, c=np.squeeze(y_train_[:,0]))
...
ind_sample = [
    dataframe[dataframe.reduced_formula == "LiCoO2"].sort_values("stability").iloc[0:2]
    ]
ind_sample = [np.argwhere(dataframe.index == ind)[0, 0] for ind in ind_sample[0].index]
...
# Sample (Lp)
samples = encoder.predict(X[ind_sample]) # train_latent[ind_sample, :]
